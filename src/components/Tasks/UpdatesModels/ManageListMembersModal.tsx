// ListMembersModal.jsx - نسخة محسنة بتصميم متطابق مع المنصة
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      if (exists) {
        toast({
          title: "Info",
          description: "Member already exists in this list",
        });
        return;
      }
      ...
      toast({
        title: "Member added",
        description: "Member has been added to the list",
      });
    } catch (err) {
      console.error(err);
      toast({
                ...
                {members.length === 0 ? (
                  <div className="rounded-2xl border-2 border-dashed border-slate-200 bg-slate-50/50 p-6 text-center sm:p-8 sm:rounded-lg">
                    ...
                    <p className="mt-0.5 text-xs text-slate-400 sm:mt-1">
                      Add members to collaborate on this list
                    </p>
                  </div>
                ) : (
                  members.map((m) => {
